...
Usage:
  stubsplit (split|merge) <stubpath> <docpath>
  stubsplit -h | --help
  stubsplit --version
    ...
    if arguments['split']:
        ...
        for root, _, files in os.walk(stubpath):
            for name in files:
                ...
                stubsplit.split(root, os.path.join(docpath, relpath), name)
    else:
        ...
        for root, _, files in os.walk(docpath):
            for name in files:
                ...
                stubsplit.combine(os.path.join(stubpath, relpath), root, name[:-3])
